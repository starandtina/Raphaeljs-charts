	...
	*/
	...
	var serializer = {
		'text': function(node) {
			...
			map(node.attrs['text'].toString().split('\n'), function(text, iterable, line) {//updated by zhangxin: add toString method to node.attrs['text']
                                line = line || 0;
				tags.push(tag(
					'text',
					reduce(
						...
						{ style: ' ' + styleToString(style) + ';' }//updated by zhangxin: remove the text-anchor: middle;https://github.com/ElbertF/Raphael.Export/issues/19
						),
					node.matrix,
					tag('tspan', { dy: computeTSpanDy(style.font.size, line + 1, node.attrs['text'].toString().split('\n').length) }, null, text)//updated by zhangxin: add toString method to node.attrs['text']
				));
			});
